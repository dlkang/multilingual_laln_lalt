...
python ${zero_path}/scripts/spm_decode.py \
...
sacrebleu $ref < $out.trans.post.txt > $out.trans.post.txt.sacrebleu
...
function handle_source {
  ...
  python ${zero_path}/scripts/spm_encode.py \
    --model ${submodel_path}/sentencepiece.model \
    --output_format=id \
    --inputs ${txt} \
    --outputs ${out}
...
zsdata_path=${opus_path}/${test_set_name}/
...
for x in ${zero_shot_langs}; do
  for y in ${zero_shot_langs}; do
    ...
    if [[ ! -d ${xy_path} ]]; then
      xy_path=${zsdata_path}/${y}-${x}/opus.${y}-${x}-test
    else
      xy_path=${xy_path}/opus.${x}-${y}-test
    fi
...
# opus en-xx translation
src_lang=en
test_set_name="supervised"
spdata_path=${opus_path}/${test_set_name}/
...
for y in $tgt_langs; do
  ...
  xy_path=${spdata_path}/${x}-${y}/
  if [[ ! -d ${xy_path} ]]; then
    xy_path=${spdata_path}/${y}-${x}/opus.${y}-${x}-test
  else
    xy_path=${xy_path}/opus.${x}-${y}-test
  fi
